      ...
      <tbody class="table-group-divider" >
        <c:forEach var="corp" items="${list}" varStatus="status">
            <tr class="align-middle">
              <td scope="row" class="text-center fw-bold">${status.count + startCount - 1 }</td>
...
$(document).ready(function () {
    ...
    $('.btnRetrieve').on('click', function(){
        ...
        AssetUtil.submitGet('/corp/list', {searchText: searchText , currentPageNumber : ${pageAttr.currentPageNumber}});
    })
...
function goPageClick(pageNo){
	 var searchText = $('#searchText').val();
     AssetUtil.submitGet('/corp/list', {searchText: searchText, currentPageNumber : pageNo });
}
</script>
